# Universal native LSP

**Status:** residual honesty (not enforced). PM **5.7**.  
**Marker:** `NATIVE_LSP_RESIDUAL_ALPHA`

## Named surface only (not product green)

- Named only: native LSP (DESIGN)
...
- No universal native LSP product path shipped


## Path A product floor (alpha) — M153

**Path A marker:** `NATIVE_LSP_PATH_A_ALPHA`  
**Status:** path A **In** — check default-deny of named residual free calls (`check_residual.oo`).  
**In:** lsp_serve free call refused at check  
**Rails:** `scripts/residual_path_a_floor_smoke.sh`  
**Still residual:** full DESIGN implementation of this moonshot (not claimed).

## Rails

- Doc marker: `NATIVE_LSP_RESIDUAL_ALPHA`
- Smoke: `scripts/native_lsp_residual_smoke.sh`
- Fixture: `fixtures/lsp_marker.oo` (marker comment only)
