...
| **Taint analysis** | **No** interprocedural taint; **no** concat/call/field propagation (`let y = x + z` still open) |
| **Other sinks** | **No** NetCap / log / file Secret gate; only `println` bare IDENT |
...
- Concat/call/field taint / full IFC / “taint tracking shipped” / “taint tracking enforced”  
...
- Fixtures: `secret_sink_fail.oo`, `secret_sink_pass.oo`, `secret_marker.oo`, `secret_assign_*`, `secret_chain_fail.oo`, `secret_concat_residual.oo`, `secret_invalid_empty.oo`
...
Concat/call taint, NetCap sinks, check dual-path, `#[Secret]` attribute grammar — still residual.
