# Compile-time FFI generation

**Status:** residual honesty (not enforced). PM **4.3.3**.  
**Marker:** `FFI_GEN_RESIDUAL_ALPHA`

## Named surface only (not product green)

- Named only: import "C" / auto wrappers (DESIGN); see also CAP_FFI.md
...
- No compile-time FFI generation product path shipped


## Path A product floor (alpha) — M153

**Path A marker:** `FFI_GEN_PATH_A_ALPHA`  
**Status:** path A **In** — check default-deny of named residual free calls (`check_residual.oo`).  
**In:** ffi_gen/import_c free calls refused at check  
**Rails:** `scripts/residual_path_a_floor_smoke.sh`  
**Still residual:** full DESIGN implementation of this moonshot (not claimed).

## Rails

- Doc marker: `FFI_GEN_RESIDUAL_ALPHA`
- Smoke: `scripts/ffi_gen_residual_smoke.sh`
- Fixture: `fixtures/ffigen_marker.oo` (marker comment only)
