# Global hive-mind fuzzing

**Status:** residual honesty (not enforced). PM **2.4**.  
**Marker:** `HIVEMIND_RESIDUAL_ALPHA`

## Named surface only (not product green)

- Named only: hive-mind / P2P overnight fuzz (DESIGN)
...
- No hive-mind / P2P fuzz product path shipped


## Path A product floor (alpha) — M153

**Path A marker:** `HIVEMIND_PATH_A_ALPHA`  
**Status:** path A **In** — check default-deny of named residual free calls (`check_residual.oo`).  
**In:** hive_fuzz/hivemind_join free calls refused at check  
**Rails:** `scripts/residual_path_a_floor_smoke.sh`  
**Still residual:** full DESIGN implementation of this moonshot (not claimed).

## Rails

- Doc marker: `HIVEMIND_RESIDUAL_ALPHA`
- Smoke: `scripts/hivemind_residual_smoke.sh`
- Fixture: `fixtures/hivemind_marker.oo` (marker comment only)
