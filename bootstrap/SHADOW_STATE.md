# Shadow-state semantic reversion

**Status:** residual honesty (not enforced). PM **3.10**.  
**Marker:** `SHADOW_STATE_RESIDUAL_ALPHA`

## Named surface only (not product green)

- Named only: shadow-state reversion (DESIGN)
...
- No shadow heap, semantic reversion, or dual-world exec shipped

## Rails

- Doc marker: `SHADOW_STATE_RESIDUAL_ALPHA`
- Smoke: `scripts/shadow_state_residual_smoke.sh`
- Fixture: `fixtures/shadow_marker.oo` (marker comment only)
