...
#   // FUZZ_REQUIRES: <fn> <expr with x[, y[, z]]>
#   // FUZZ_ENSURES: <fn> <expr with x[, y[, z]] and/or result>
# M49: Int arity-2/3 → __fuzz_x[/y[/z]]; arity≥4 / multi-arg non-int fail-closed
set -euo pipefail
ROOT_FUZZ="$(cd "$(dirname "$0")" && pwd)"
# shellcheck source=ooda_fuzz_pure_arity.sh
source "$ROOT_FUZZ/ooda_fuzz_pure_arity.sh"
...
# requires/ensures keyed by fn name (AND multi-// markers for same fn)
declare -A REQ_EXPR=()
declare -A ENS_EXPR=()
while IFS= read -r line; do
  ...
  fn="$1"
  shift
  if [[ -n "${REQ_EXPR[$fn]+x}" && -n "${REQ_EXPR[$fn]}" ]]; then
    REQ_EXPR["$fn"]="(${REQ_EXPR[$fn]}) && ($*)"
  else
    REQ_EXPR["$fn"]="$*"
  fi
...
fuzz_strip_body "$SRC" "$body_tmp"
if ! grep -qE '^(pub[[:space:]]+)?fn[[:space:]]+' "$body_tmp"; then
  echo "ERR	fuzz	no non-main functions to fuzz in $SRC" >&2
  exit 2
fi
# Per-target arity from signature (M49: Int arity-2/3 In; ≥4 fail-closed)
ARITIES=()
for fname in "${TARGETS[@]}"; do
  a="$(fuzz_fn_arity "$fname" "$body_tmp")" || exit $?
  fuzz_check_arity "$fname" "$a" "$body_tmp" || exit $?
  ARITIES+=("$a")
done

{
  ...
  for fname in "${TARGETS[@]}"; do
    ...
    ens_chk=""
    if [[ -n "$req" ]]; then
      req_chk="$(fuzz_rewrite_expr "$req" req)"
    fi
    if [[ -n "$ens" ]]; then
      ens_chk="$(fuzz_rewrite_expr "$ens" ens)"
