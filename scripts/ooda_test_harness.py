...
def main() -> int:
            ...
            while pos < blen:
                ...
                if b.startswith("//", pos):
                    while pos < blen and b[pos] != "\n":
                        pos += 1
                    continue
                m2 = re.match(r"assert_eq!\s*\(", b[pos:])
                if not m2:
                    snippet = b[pos : pos + 48].replace("\n", " ")
                    print(
                        f"ERR\ttest\tverify {vname}: only assert_eq! supported ({snippet!r})",
                        file=sys.stderr,
                    )
                    return 1
                pos = pos + m2.end() - 1
                ...
                try:
                    ...
                    return 1
                aidx += 1
                asserts.append((lhs, rhs, vname, aidx))
                pos = end
                while pos < blen and b[pos] in " \t\r\n":
                    pos += 1
    ...
    if not asserts:
        print("ERR\ttest\tverify blocks present but no assert_eq!", file=sys.stderr)
        return 1
    ...
    parts.append("\npub fn main() {\n")
    for lhs, rhs, vname, aidx in asserts:
        parts.append(f"    if {lhs} != {rhs} {{\n")
