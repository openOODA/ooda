...
fn ooda_wasm_list_sum_fixture_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let src = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/list_sum.oo");
    assert!(src.is_file(), "missing {}", src.display());
    let dir = unique_temp_dir("ooda_wlist_sum");
    let path = dir.join("list_sum.oo");
    std::fs::copy(&src, &path).expect("copy fixture");
    let out = Command::new(bin)
        .args(["build", "--target", "wasm", path.to_str().unwrap()])
        .output()
...
#[test]
fn ooda_wasm_int_lt_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let dir = unique_temp_dir("ooda_wilt");
    let path = dir.join("ilt.oo");
    std::fs::write(
        &path,
        r#"
pub fn main() {
    if 1 < 2 { println(1); } else { println(0); }
    if 3 < 2 { println(1); } else { println(0); }
}
"#,
    )
    .unwrap();
    ...
    let wat = std::fs::read_to_string(path.with_extension("wat")).unwrap();
    let lines = run_wat(&wat).expect("host lt");
    assert_eq!(
        lines,
        vec!["1".to_string(), "0".to_string()],
        "got {:?}",
...
/// Dual-engine: Int equality under host.
#[test]
fn ooda_wasm_int_eq_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let dir = unique_temp_dir("ooda_wieq");
    let path = dir.join("ieq.oo");
    std::fs::write(
        &path,
        r#"
pub fn main() {
...
"#,
    ...
    assert!(out.status.success());
    let wat = std::fs::read_to_string(path.with_extension("wat")).unwrap();
    let lines = run_wat(&wat).expect("host eq");
    assert_eq!(lines, vec!["1".to_string(), "0".to_string()], "got {:?}", lines);
    let _ = std::fs::remove_dir_all(&dir);
}
...
/// Dual-engine: Int greater-than under host.
#[test]
fn ooda_wasm_int_gt_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let dir = unique_temp_dir("ooda_wigt");
    let path = dir.join("igt.oo");
    std::fs::write(
        &path,
        r#"
pub fn main() {
...
/// Dual-engine: Int less-or-equal under host.
#[test]
fn ooda_wasm_int_lte_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let dir = unique_temp_dir("ooda_wilte");
    let path = dir.join("ilte.oo");
    std::fs::write(
        &path,
        r#"
pub fn main() {
...
/// Dual-engine: Int greater-or-equal under host.
#[test]
fn ooda_wasm_int_gte_runs_on_host() {
    let bin = env!("CARGO_BIN_EXE_ooda");
    let dir = unique_temp_dir("ooda_wigte");
    let path = dir.join("igte.oo");
    std::fs::write(
        &path,
        r#"
pub fn main() {
