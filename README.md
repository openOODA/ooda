...
**openOODA Project** — `https://github.com/openOODA` — **Version `v0.92.0-alpha`**
...
## What's real in v0.92.0-alpha (CHS M0–M5)
...
| WASM strings (subset) | Real: interned literals → data segment + `println_str`; `==`/`!=` via host `env.streq` (NUL-terminated); **concat/arith refuse**. Not methods / lists / dynamic alloc |
...
| LSP live diagnostics | Real Incremental `textDocumentSync` (kind 2) + open-buffer store; full replace still accepted. **WorkspaceEdit** `codeAction`: `let mut`, missing return, return-type, undefined-var stub, missing `: Int` (not completion/hover/rename) |
