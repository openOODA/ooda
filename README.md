...
**openOODA Project** — `https://github.com/openOODA` — **Version `v0.101.0-alpha`**
...
## What's real in v0.101.0-alpha (CHS M0–M5)
...
| WASM strings + List[Int] | Real: strings + `streq` + `.len`/`.char_at` (pure WAT); List[Int] free+method. Refuse List[String]/caps/most methods. Fixture `fixtures/string_walk.oo`. Dev wasmtime smoke |
