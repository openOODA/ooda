...
**openOODA Project** — `https://github.com/openOODA` — **Version `v0.115.0-alpha`**
...
## What's real in v0.115.0-alpha (CHS M0–M5)
...
| **oodac** (`oodac/main.oo`) | Real lex/parse/cap-check + **R1 typecheck slice** (annotated let + return literals fail-closed in `.oo`); build hybrid. Not full typecheck/eval self-host |
...
oodac typecheck is a **literal/annotation slice only** — not full `src/typecheck.rs` parity.  
