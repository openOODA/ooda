...
**openOODA Project** — `https://github.com/openOODA` — **Version `v0.90.0-alpha`**
...
## What's real in v0.90.0-alpha (CHS M0–M5)
...
| WASM strings (subset) | Real: interned literals → data segment + `println_str`; `==`/`!=` pointer identity; **concat/arith refuse** (no silent pointer math). Not content-eq / methods / lists |
...
| LSP live diagnostics | Real via `textDocument/didOpen` & `textDocument/didChange` (open-buffer store). **WorkspaceEdit** `codeAction` for `let mut`, missing-return inserts, return-type fix (not completion/hover/rename) |
