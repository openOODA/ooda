...
/* Walk colon-separated absolute dirs. fn==NULL counts only. Returns 0 or -errno-class. */
...
OoResS oo_landlock_restrict(long long cap, OoStr read_dirs, OoStr write_dirs) {
  ...
  if (nread + nwrite <= 0) return ll_err("ERR\tlandlock\tempty path allowlist");
  if (nread + nwrite > LL_MAX_DIRS) return ll_err("ERR\tlandlock\ttoo many paths");

#if !defined(__NR_landlock_create_ruleset)
  return ll_err("ERR\tlandlock\tsyscall ABI unavailable");
#else
  int abi = ll_abi_version();
  if (abi < 1) return ll_err("ERR\tlandlock\tsyscall ABI unavailable");
  ...
  if (nread > 0 && ll_each_dir(read_dirs, NULL, ll_add_one, &ctx) != 0) {
    close(ruleset_fd);
    return ll_err("ERR\tlandlock\tadd_rule read path failed");
  }
  ctx.access = ll_write_bits(abi) & attr.handled_access_fs;
  if (nwrite > 0 && ll_each_dir(write_dirs, NULL, ll_add_one, &ctx) != 0) {
    close(ruleset_fd);
